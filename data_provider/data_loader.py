...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        self.gap = gap * self.steps_per_day
    ...
    def __read_data__(self):
        ...
        months = df_stamp['date'].dt.month.values.astype(np.float32)
        weekday = df_stamp['date'].dt.weekday.values.astype(np.float32)
        year_value = years
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp.date.apply(lambda row: row.month, 1)
            df_stamp['day'] = df_stamp.date.apply(lambda row: row.day, 1)
            ...
            data_stamp = data_stamp.transpose(1, 0)
            if self.camel_mark:
                data_stamp = np.concatenate([data_stamp,weekday.reshape(-1, 1),months.reshape(-1, 1), year_value.reshape(-1, 1)], axis=1)
    ...
    def __getitem__(self, index):
        s_begin = index
        s_end = s_begin + self.seq_len
        r_begin = s_end + self.gap - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
    ...
    def __init__(self, root_path, flag='pred', size=None,
        ...
        if M4Dataset is None or M4Meta is None:
            raise ModuleNotFoundError(
                "M4 support requires optional dependency 'patoolib'. "
                "Please install it before using --data m4."
            )
        self.features = features
        self.target = target
        self.scale = scale
...
class UEAloader(Dataset):
    ...
    def load_single(self, filepath):
        if load_from_tsfile_to_dataframe is None:
            raise ModuleNotFoundError(
                "UEA support requires optional dependency 'sktime'. "
                "Please install it before using UEA datasets."
            )
