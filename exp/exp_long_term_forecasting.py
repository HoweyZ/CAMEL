...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _compute_total_loss(self, batch_x, dec_inp, batch_y, batch_x_mark, batch_y_mark, criterion):
        ...
        if self.args.model == 'CAMEL':
            ...
            self.loss_print_count += 1
            if self.loss_print_count % 100 == 0:
                length = int(task_loss.item() * 10)
                length = min(length, 50)
                bar = "█" * length + "-" * (50 - length)
                print(f"loss: {task_loss.item():.4f} | [{bar}]")
    ...
    def test(self, setting, test=0):
        ...
        with open('result_for_super.txt', 'a') as f:
            f.write(setting + '  \n')
